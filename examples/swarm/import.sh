#!/usr/bin/env bash
# Import the fork's agent-swarm combos into a running OmniRoute.
#
# Usage:
...
#
# The combos use the `swarm` strategy: each task in config.swarm.tasks is
# executed by its own worker (explicit model or tag-resolved specialist) in
# parallel. Tag specs re-resolve on every dispatch, so the swarm tracks the
# catalog automatically. ad-hoc-swarm is meant to be driven per request via
# `body.swarm.tasks` (see examples/swarm/README.md).
set -euo pipefail
...
echo "Importing agent-swarm combos into ${BASE_URL} ..."
